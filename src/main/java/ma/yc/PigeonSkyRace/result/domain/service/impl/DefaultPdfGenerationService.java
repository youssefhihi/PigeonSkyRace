...
public class DefaultPdfGenerationService implements PdfGenerationService {
    ...
    private PdfPTable createResultsTable ( List<ResultResponseDto> results ) throws DocumentException {
        ...
        float[] columnWidths = {2f, 2f, 2f, 1.5f, 2.5f};
        ...
        for (int i = 0; i < results.size(); i++) {
            ...
            addTableCell(table, result.bandNumber().toString(), dataFont, background);
            addTableCell(table, String.format("%.2f", result.distance()), dataFont, background);
            addTableCell(table, String.format("%.2f", result.speed()), dataFont, background);
            addTableCell(table, String.format("%.2f", result.points()), dataFont, background);
            addTableCell(table, result.dateArrival().format(DATE_FORMATTER), dataFont, background);
        }
